  ...
  ({className, ...props}, ref) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      const result = await uploadListing(
        formData,
        formData.listingDocuments || [], // Assuming you have documents in your form data
        formData.listingImages || []     // Assuming you have images in your form data
      );
    ...
    return (
      <>
        <div className="text-3xl font-bold leading-9 text-default-foreground">Review & Contact Information</div>
        ...
        <div className="mb-6 rounded-lg bg-content1 p-6">
          ...
          <div className="grid gap-4">
            <div>
              ...
              <p className="text-base">${formData.loanAmount?.toLocaleString()}</p>
            </div>
            <div>
            ...
            {formData.fullAddressDetails && (
              <>
                <div className="mt-4 border-t pt-4">
                  <p className="text-sm font-semibold text-default-500">Property Location</p>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-sm text-default-500">
                    <div>
                      <p>City</p>
                      <p className="text-default-700">{formData.fullAddressDetails.city}</p>
                    </div>
                    <div>
                      <p>Province</p>
                      <p className="text-default-700">{formData.fullAddressDetails.province}</p>
                    </div>
                    <div>
                      <p>Postal Code</p>
                      <p className="text-default-700">{formData.fullAddressDetails.postalCode}</p>
                    </div>
                    <div>
                      <p>Country</p>
                      <p className="text-default-700">{formData.fullAddressDetails.country}</p>
                    </div>
                  </div>
                </div>
              </>
        ...
        >
          ...
          <Autocomplete
            ...
            onSelectionChange={(key) => updateFormData({ country: key as string })}
            required
          ...
          <Select
            ...
            onSelectionChange={(keys) => updateFormData({ state: Array.from(keys)[0] as string })}
            required
            {...selectProps}
          >
            {(registrationState) => (
