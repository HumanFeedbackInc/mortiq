...
const PriceSlider = React.forwardRef<HTMLDivElement, PriceSliderProps>(
  ({ range, animation, className, displayFormat = "usd", ...props }, ref) => {
    const defaultValue = React.useMemo<RangeValue>(
      () =>
        range?.defaultValue ||
        (displayFormat === "percent" ? [0, 100] : [0, 1000]),
      [range?.defaultValue, displayFormat]
    );

    const [value, setValue] = React.useState<RangeValue>(
      Array.isArray(props.value) ? (props.value as RangeValue) : defaultValue
    );
    ...
    const onMinInputValueChange = React.useCallback(
      (inputValue: string) => {
        ...
        if (!isNaN(newValue)) {
          ...
          const updated = [clampedValue, value[1]] as RangeValue;
          setValue(updated);
          if (props.onChange) {
      ...
      [value, range?.min, defaultValue, props]
    );
    ...
    const onMaxInputValueChange = React.useCallback(
      (inputValue: string) => {
        ...
        if (!isNaN(newValue) && newValue <= maxValue) {
          const updated = [value[0], newValue] as RangeValue;
          setValue(updated);
          if (props.onChange) {
            props.onChange(updated);
          }
      ...
      [value, range?.max, defaultValue, props]
    );

    const formatValue = (val: number) => {
      if (displayFormat === "usd") {
    ...
    const sliderFormatOptions: Intl.NumberFormatOptions =
      displayFormat === "usd"
        ? {
      ...
      <div className={cn("flex flex-col gap-3", className)}>
        <div className="flex flex-col gap-1">
          ...
          <Slider
            {...props}
            ref={ref}
            formatOptions={sliderFormatOptions}
            maxValue={range?.max ?? (displayFormat === "percent" ? 100 : 1000)}
            minValue={range?.min ?? (displayFormat === "percent" ? 0 : 0)}
            size="sm"
            step={range?.step}
            value={value}
            onChange={(newValue) => {
              const castedValue = newValue as RangeValue;
              setValue(castedValue);
              if (props.onChange) {
                props.onChange(castedValue);
              }
            }}
          />
        </div>
        <div className="flex items-center">
          <Input
            aria-label={displayFormat === "usd" ? "Min price" : "Min percent"}
            labelPlacement="outside"
            {...(displayFormat === "usd"
              ? { startContent: <p className="text-default-400">$</p> }
              : { endContent: <p className="text-default-400">%</p> })}
            type="number"
            value={`${value[0]}`}
          ...
          <Input
            aria-label={displayFormat === "usd" ? "Max price" : "Max percent"}
            labelPlacement="outside"
            {...(displayFormat === "usd"
              ? { startContent: <p className="text-default-400">$</p> }
              : { endContent: <p className="text-default-400">%</p> })}
